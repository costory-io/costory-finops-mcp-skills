...
4. `suggest_groupby` on chosen period + scope → build candidate trees (root + 1–2 deeper levels each)
   - If suggestions are empty / weak → fall back to `popularGroupBys`, else `cos_provider` → `cos_service_name`
   - **Always** propose **3 distinct trees** (different roots or first-level splits from `suggest_groupby` / `popularGroupBys`) in one short list and **wait for the user to pick** before step 5. Example shapes: provider → service; team → service; env → service — adapt to what the org actually has. Do not preview until they choose.
   - **NOW / delivery:** reconfirm the chosen path before create
...
**Chat-only explain (default):** do **not** block on a long questionnaire. Defaults — whole org, `LAST_MONTH`, tree-only AI off. **Always** ask the user to pick among **3 trees** before preview. Also ask when scope/period is ambiguous or they want AI / NOW.
...
- Do **not** skip the tree pick — always suggest 3 trees and wait for the user before `preview_report_widget`.
